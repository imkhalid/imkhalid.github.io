...
fun AddEditVendorScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.event.collect { event ->
            when (event) {
                is AddEditVendorEvent.SaveSuccess -> {
                    Toast.makeText(
                        context,
                        if (uiState.isEditMode) "Vendor updated" else "Vendor added",
                        Toast.LENGTH_SHORT
                    ).show()
                    onNavigateBack()
                }

                is AddEditVendorEvent.ShowError -> {
        ...
        if (uiState.isLoading) {
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("Address") },
                    placeholder = { Text("Vendor address") },
                    maxLines = 3,
                    keyboardOptions = KeyboardOptions(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = viewModel::onBankDetailsChanged,
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("Bank Details") },
                    placeholder = { Text("Account number, bank name, etc.") },
                    maxLines = 3,
                    keyboardOptions = KeyboardOptions(
                        capitalization = KeyboardCapitalization.Sentences
                    )
